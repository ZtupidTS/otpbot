...
Global Const $_BIGNUM_MAXLEN=4096
...
Func _BigNum_Pow($n, $e = 2)
	$e = Number($e)
	If IsInt($e) = 0 Then Return SetError(2, 0, 'ERROR_INT')

	If _BigNum_Compare($e,0)=-1 Then; Added by CrashDemons
		Return _BigNum_Div(1,_BigNum_Pow($n,-$e))
	EndIf



	;If $e < -2147483648 Or $e > 2147483647 Then Return SetError(-2, 0, -1)
	If Not __BigNum_IsValid($n, $n) Then Return SetError(1, 0, 'ERROR_INVALIDINPUT')
...
Func _BigNum_SQRT($n, $p = -1)
	If Not __BigNum_IsValid($n, $n) Then Return SetError(-1, 0, 'ERROR_INVALIDINPUT')
	$p = Number($p)
	If IsInt($p) = 0 Or $p < -1 Then Return SetError(-2, 0, 'ERROR_NATURAL')
	...
	Next
	Return SetError(-3, 0, 'ERROR_EXHAUSTION')
...
Func _BigNum_n_Root($n, $e = 2, $p = -1)
	If Not __BigNum_IsValid($n, $n) Then Return SetError(1, 0, 'ERROR_INVALIDINPUT')
	$e = Number($e)
	If IsInt($e) = 0 Or $e < 1 Then Return SetError(2, 0, 'e:ERROR_WHOLE')
	$p = Number($p)
	If IsInt($p) = 0 Or $p < -1 Then Return SetError(3, 0, 'p:ERROR_NATURAL')
...
Func _BigNum_Round($sX, $iD)
	If Not __BigNum_IsValid($sX, $sX) Then Return SetError(1, 0, 'ERROR_INVALIDINPUT')
...
Func __BigNum_IsValid($sX, $sY)
	;If StringLen($sX)>$_BIGNUM_MAXLEN Or StringLen($sY)>$_BIGNUM_MAXLEN Then Return False
